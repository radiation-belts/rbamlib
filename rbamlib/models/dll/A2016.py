...
def A2016(L, kp, dll_type='M'):
    ...
    dllm, dlle : tuple of numpy.ndarray
        Both magnetic and electric diffusion coefficients (dllm, dlle) if dll_type='ME', 'BE', 'both', or ''.
    ...
    .. math::
        D^{B}_{LL}[RBSP] = \exp \left( a_1 + b_1 \cdot Kp \cdot L^* + L^* \right)

    The electric radial diffusion coefficient is calculated as:

    .. math::
        D^{E}_{LL}[RBSP] = \exp \left( a_2 + b_2 \cdot Kp \cdot L^* + c_2 \cdot L^* \right)

    where the constants are given by:

    .. math::
        a_1 = -16.253, \quad b_1 = 0.224

    .. math::
        a_2 = -16.951, \quad b_2 = 0.181, \quad c_2 = 1.982
    ...
    if dll_type in {'M', 'm', 'B', 'b', '', 'ME', 'me', 'BE', 'be', 'both'}:
        dllm = np.exp(-16.253 + 0.224 * kp * L + L)
