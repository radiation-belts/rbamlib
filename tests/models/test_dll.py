...
class TestDll(unittest.TestCase, TestHelpers):
    ...
    def test_BA2000_values(self):
        ...
        expected_dllm = np.array([0.00510035, 0.0475007, 0.29411184])
        np.testing.assert_almost_equal(dllm, expected_dllm, decimal=4,
        ...
        expected_dlle = np.array([8.4195e-03, 4.3609e-02, 1.7471e-01])
        np.testing.assert_almost_equal(dlle, expected_dlle, decimal=4,
                                       err_msg="Electromagnetic diffusion coefficients are incorrect.")

        # Test both diffusion coefficients
        dllm_both, dlle_both = BA2000(self.L, self.kp, mu=self.mu, dll_type='ME')
        np.testing.assert_almost_equal(dllm_both, expected_dllm, decimal=4,
                                       err_msg="Electromagnetic coefficients from 'ME' type are incorrect.")
        np.testing.assert_almost_equal(dlle_both, expected_dlle, decimal=4,
