...
def TS2005_W(time, S, storm_onsets=None, fill_value=np.nan):
    ...
    if not storm_onsets:
        storm_onsets = [0, n_times]
    else:
        # Ensure sorted, unique
        storm_onsets = sorted(set(storm_onsets))
        # Filter any that are out-of-bounds
        storm_onsets = [s for s in storm_onsets if 0 <= s < n_times]
        # Append a sentinel at the end if needed
        if storm_onsets[-1] < n_times:
            storm_onsets.append(n_times)

    # Process each storm segment separately
    for seg in range(len(storm_onsets) - 1):
        idx_start = storm_onsets[seg]
        idx_end = storm_onsets[seg + 1]  # not included in the segment
