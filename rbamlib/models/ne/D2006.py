...
def D2006_plasmasphere(L, MLT=0.0, r=None):
    r"""
    ...
    Notes
    -----
    Denton et al. (2006) defined an empirical **equatorial plasmasphere model**
    with explicit MLT variation. Using
    :math:`\phi = 2\pi(\mathrm{MLT}-18)/24`, the two regimes are:
...
def D2006_trough(L):
    """
    Denton et al. (2006) [#]_ trough density fit.
    ...
    .. math::
        \log_{10} n_e = 5.25 - 0.82\,L

    where :math:`n_e` is the electron density in :math:`\text{cm}^{-3}`
    and :math:`L` is the McIlwain L-shell parameter.
    ...
    """
    L = np.asarray(L, dtype=float)
    log10_ne = 5.25 - 0.82 * L
    return 10.0 ** log10_ne
